...
public class TourDB {
    ...
    public static List<ChiTietTour> searchTourNangCao(String diemdenSearch, String phuongtienSearch, String giaSearch) {
        List<ChiTietTour> resultList = new ArrayList<>();
        if (diemdenSearch == null || diemdenSearch.trim().equals("") || phuongtienSearch == null || phuongtienSearch.trim().equals("")
                || giaSearch == null || giaSearch.trim().equals("")) {
            return resultList;
        }
        String sql = "select * from tour where 1=1 ";
        if (diemdenSearch != null) {
            sql += " AND tentour like ? ";
        }

        if (phuongtienSearch != null) {
            sql += " AND phuongtien = ? ";
        }
